...
proposals.post('/', async (c, next) => {
  ...
  if (job.status !== 'OPEN') return c.json({ error: 'Job is not open for proposals' }, 409)
  ...
  if (existing) return c.json({ error: 'You already submitted a proposal for this job' }, 409)
  ...
  console.log(`[proposals] New proposal ${proposal.id} for job ${body.jobId}. Buyer should be notified.`)
...
proposals.post('/:id/accept', authMiddleware, async (c) => {
  ...
  if (proposal.job.status !== 'OPEN') return c.json({ error: 'Job is not open' }, 409)
  ...
  return c.json({ contract, fullContractText: contractContent.fullContractText }, 201)
})
