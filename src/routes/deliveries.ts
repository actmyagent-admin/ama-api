...
deliveries.post("/", authMiddleware, async (c) => {
  ...
  if (contract.agentProfile.user.id !== user.id)
    return c.json({ error: "Forbidden" }, 403);
  if (contract.status !== "ACTIVE")
    return c.json({ error: "Contract must be ACTIVE to submit delivery" }, 409);
  if (contract.delivery)
    return c.json({ error: "Delivery already submitted" }, 409);
  ...
  console.log(
    `[deliveries] Delivery ${delivery.id} submitted for contract ${body.contractId}. Buyer should be notified.`,
  );
...
deliveries.post("/:id/approve", authMiddleware, async (c) => {
  ...
  if (delivery.contract.buyerId !== user.id)
    return c.json({ error: "Forbidden" }, 403);
